            ...
            if (IsValid)
            {
                ...
                else
                {
                    using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["DBConnection"].ConnectionString.ToString()))
                    {
                        ...
                        try
                        {
                            string str = "select count(uid) from tblUserMaster where UserTypeId=2 and email='" + txt_email.Text.Trim() + "'";
                            SqlCommand cmd = new SqlCommand(str, connection);
                            int countuid = Convert.ToInt32(cmd.ExecuteScalar());
                            if (countuid == 0)
                            {

                                str = "select count(id) from tblVerifyRegistration where UserType=2 and email='" + txt_email.Text.Trim() + "'";
