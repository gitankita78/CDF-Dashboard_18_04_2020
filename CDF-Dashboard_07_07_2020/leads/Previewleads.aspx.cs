...
public partial class Candidate_Previewleads : System.Web.UI.Page
{
    ...
    private void bindgrid()
    {
        ...
        try
        {
            ...
            {
                //Show preview leads of respective user
                string strcmd = "select FName as'First Name',LName as'Last Name',ContactNo as 'Contact Number'," +
                                   "email as 'Email Address',AddedDate as 'Date',LeadSource as 'Lead Source'," +
                                   "LeadStatus as 'Lead Status', " +
                                   "CONCAT(FName, ' ', LName) as Assigned_User FROM BitrixLeads where ReferedByEmail = '" + Session["dheyaEmail"].ToString() + "'";

                if (txt_name.Text != "")
                {
                    strcmd += " AND FName like '%" + txt_name.Text + "%'";
                }
                strcmd += " order by AddedDate desc";
