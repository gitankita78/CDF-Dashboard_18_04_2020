...
public partial class CDF_CDF_reports : System.Web.UI.Page
{
            ...
            if (c_id > 0)
            {
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    ...
                    if (sdruserinfo.HasRows)
                    {
                        ...
                        strcmd = "SELECT rating,P_rating,A.factor_no,B.factor FROM tblKYCandFactors as A,tblKYFactors as B where A.factor_no=B.factor_no and A.batid=" + batid + " and c_id = " + c_id + " order by P_rating desc";
                        DataSet ds1 = dal.ExecDataSet(strcmd);

                        strcmd = "SELECT rating,P_rating,A.factor_no,B.factor FROM tblKYCandFactors as A,tblKYFactors as B where A.factor_no=B.factor_no and A.batid=" + batid + " and c_id = " + c_id + " order by A.factor_no";
    ...
    private void ps1()
    {
        ...
        strcmd = "select rating,P_rating,factor_no from tblKYCandFactors where batid=" + batid + " and c_id = " + c_id + " order by P_rating";
        //strcmd = "select * from tbl_KY_cand_factors where c_id = '" + c_id + "'order by factor_no";
        DataSet ds1 = dal.ExecDataSet(strcmd);

        // Chart1.TempDirectory = "~/MobileAppReports/Reports_graph"; //Server.MapPath("~/images");
        Chart1.TempDirectory = "~/Reports_graph";
        Chart1.Debug = true;
